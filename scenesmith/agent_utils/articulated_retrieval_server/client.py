"""Client for making requests to the articulated retrieval server."""
...
class ArticulatedRetrievalClient:
    ...
    Provides a high-level interface for retrieving articulated 3D objects
    from PartNet-Mobility and ArtVIP datasets using semantic search with CLIP.
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 7002):
        """Initialize articulated retrieval client.

        Args:
            host: Server hostname or IP address. Should be accessible from
                the current network context. Defaults to localhost.
            port: Server port number. Must match the port where the articulated
                retrieval server is listening. Defaults to 7002.
        """
    ...
    def retrieve_objects(
        ...
        Submits a batch of articulated retrieval requests to the server and yields
        ...
        Args:
            retrieval_requests: List of articulated retrieval requests to process
                as a batch.
            ...
            try:
                    ...
                    if line:
                        try:
                            ...
                            if streamed_result.status == "error":
                                raise RuntimeError(
                                    f"Articulated retrieval failed for request "
                            ...
                            response = ArticulatedRetrievalServerResponse.from_dict(
                ...
                if attempt < max_retries - 1:
                    ...
                    console_logger.error(
                        "Articulated retrieval server connection failed after retries"
                    )
                    raise ConnectionError(
                        f"Failed to connect to articulated retrieval server at "
                        f"{self.base_url}"
                    ) from e
                ...
                try:
                    ...
                    error_detail = str(e)
                console_logger.error(
                    f"HTTP error from articulated retrieval server: {error_detail}"
                )
                raise RuntimeError(
                    f"Articulated retrieval server error: {error_detail}"
                ) from e
    ...
    def health_check(self) -> bool:
        """Check if the articulated retrieval server is healthy and responsive.
