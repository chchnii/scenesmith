...
class HssdRetrievalClient:
    """Client for making requests to the HSSD retrieval server.

    Provides a high-level interface for retrieving 3D objects from the HSSD
    library using semantic search with CLIP. Handles HTTP communication,
    ...
    Example:
        >>> client = HssdRetrievalClient()
        >>> requests = [HssdRetrievalServerRequest(
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 7001):
        """Initialize HSSD retrieval client.

        Args:
            host: Server hostname or IP address. Should be accessible from
                the current network context. Defaults to localhost.
            port: Server port number. Must match the port where the HSSD
                retrieval server is listening. Defaults to 7001.
        """
    ...
    def retrieve_objects(
        ...
        Submits a batch of HSSD retrieval requests to the server and yields
            ...
            try:
                ...
                for line in http_response.iter_lines():
                    if line:
                        try:
                            ...
                            response = HssdRetrievalServerResponse.from_dict(
                ...
                if attempt < max_retries - 1:
                    ...
                    console_logger.error(
                        "HSSD retrieval server connection failed after retries"
                    )
                    raise ConnectionError(
                        f"Failed to connect to HSSD retrieval server at "
                        f"{self.base_url}"
                    ) from e
